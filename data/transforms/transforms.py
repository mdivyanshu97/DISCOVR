...
class GroupNormalize(object):
    ...
    def __call__(self, tensor_tuple):
        ...
        if tensor.size(1) == 1:  # Grayscale
            rep_mean = np.array(self.mean).mean()  # Use first mean value for grayscale
            rep_std = np.array(self.std).mean()    # Use first std value for grayscale
        else:  # RGB
...
class GroupGrayscale(object):
    """Convert all frames in a clip to grayscale."""
    def __init__(self):
    ...
    def __call__(self, img_tuple):
        ...
        if isinstance(img_group[0], np.ndarray):
            # Convert numpy arrays to grayscale
            gray_images = []
            for img in img_group:
                if img.shape[2] == 3:  # RGB
                    gray = 0.299 * img[:,:,0] + 0.587 * img[:,:,1] + 0.114 * img[:,:,2]
                    gray = np.expand_dims(gray, axis=2)  # Add channel dimension
                    gray_images.append(gray)
                else:  # Already grayscale
                    if len(img.shape) == 2:  # If (H,W)
                        gray = np.expand_dims(img, axis=2)  # Add channel dimension
                    else:  # If already (H,W,1)
                        gray = img
                    gray_images.append(gray)
            return (gray_images, label)
        else:
            # Handle PIL Images
            return ([self.grayscale(img) for img in img_group], label)
...
class Stack(object):
    ...
    def __call__(self, img_tuple):
        ...
        if isinstance(img_group[0], np.ndarray):
            # For numpy arrays, handle both grayscale and RGB
            print('img_group',img_group[0].shape)
            if img_group[0].shape[2] == 1:  # Grayscale
                # Stack along the channel dimension to get (H,W,T)
                stacked = np.stack([x[:,:,0] for x in img_group], axis=2)
                # Reshape to (T,H,W,1)
                stacked = np.expand_dims(stacked, axis=3)
                print('stacked',stacked.shape)
                return (stacked, label)
            else:  # RGB
                if self.roll:
                    return (np.concatenate([x[:, :, ::-1] for x in img_group], axis=2), label)
                else:
...
class ToTorchFormatTensor(object):
    ...
    def __call__(self, pic_tuple):
        ...
        if isinstance(pic, np.ndarray):
            # handle numpy array - same operation for both grayscale and RGB
